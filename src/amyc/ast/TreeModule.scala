...
trait TreeModule {
  ...
  // Update :: Local val definition
  case class Let(df: ParamDef, value: Expr, body: Expr, variable: Boolean) extends Expr
  // Update :: Variable reassign
  case class Reassign(name: Name, newValue: Expr) extends Expr
  // Update :: Reassign operators
  case class PlusEquals(name: Name, value: Expr) extends Expr
  case class MinusEquals(name: Name, value: Expr) extends Expr
  case class TimesEquals(name: Name, value: Expr) extends Expr
  case class DivEquals(name: Name, value: Expr) extends Expr
  case class ModEquals(name: Name, value: Expr) extends Expr
  case class ConcatEquals(name: Name, value: Expr) extends Expr
  // If-then-else
